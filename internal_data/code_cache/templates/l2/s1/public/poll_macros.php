<?php
// FROM HASH: 518ae2c0bbadf2b57f83eea44f90e94b
return array(
...
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	if ($__vars['poll']['max_votes'] != 1) {
		$__compilerTemp3 .= '
						<li>' . 'Можно выбрать сразу несколько вариантов.' . '</li>
					';
	}
	$__compilerTemp3 .= '
					';
	...
	if ($__templater->method($__vars['poll'], 'canViewResults', array())) {
		$__finalCompiled .= '
				' . $__templater->button('Посмотреть результаты', array(
...
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	if ($__vars['showFooter']) {
		...
		if ($__vars['simpleDisplay']) {
			$__compilerTemp1 .= '
					<a href="' . $__templater->escape($__templater->method($__vars['poll'], 'getLink', array('content', ))) . '">' . 'Посмотреть комментарии' . $__vars['xf']['language']['ellipsis'] . '</a>
				';
		} else {
			$__compilerTemp1 .= '
					';
...
'code' => function($__templater, array $__vars, $__extensions = null)
{
			...
			foreach ($__vars['poll']['Responses'] AS $__vars['response']) {
				...
				if ($__templater->method($__vars['poll'], 'canEditDetails', array())) {
					$__compilerTemp1 .= '
						...
						'aria-label' => $__templater->filter('Вариант ответа', array(array('for_attr', array()),), false),
	...
	$__finalCompiled .= $__templater->formRadioRow(array(
		...
		'explain' => 'Максимальное число ответов, которое пользователь может выбрать при голосовании.',
		'rowtype' => $__vars['rowType'],
	)) . '
	...
	if ((!$__vars['poll']) OR $__templater->method($__vars['poll'], 'canChangePollVisibility', array())) {
		$__compilerTemp4[] = array(
			...
			'label' => '
				' . 'Разрешить всем видеть, кто и как проголосовал' . '
			',
			'_type' => 'option',
		);
	}
	$__compilerTemp4[] = array(
		...
		'label' => '
			' . 'Разрешить видеть результаты без участия в опросе' . '
		',
		'_type' => 'option',
	);
