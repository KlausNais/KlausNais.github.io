...
'thread_list_header' => function($__templater, array $__vars, $__extensions = null)
{
	...
	if ($__templater->method($__vars['forum'], 'canWatch', array())) {
		...
		if ($__vars['forum']['Watch'][$__vars['xf']['visitor']['user_id']]) {
			$__compilerTemp2 .= 'Прекратить отслеживание';
		} else {
			$__compilerTemp2 .= 'Отслеживать';
		}
	...
	$__finalCompiled .= '
					...
					<a class="filterBar-menuTrigger" data-xf-click="menu" role="button" tabindex="0" aria-expanded="false" aria-haspopup="true"><i class="fad fa-filter"></i>' . 'Фильтры' . '</a>
	...
	if ($__templater->func('property', array('dt_disc_minor_header', ), false)) {
		$__finalCompiled .= '
						<div class="discu-minorHeader">
							<span class="discu-minorHeader--main">' . 'Заголовок' . '</span>
							<span class="discu-minorHeader--stats">' . 'Ответы' . '</span>
							<span class="discu-minorHeader--stats">' . 'Просмотры' . '</span>
							';
		if ($__templater->func('property', array('dt_reactions_column', ), false)) {
			$__finalCompiled .= '
								<span class="discu-minorHeader--stats">' . 'Реакции' . '</span>
							';
		}
		$__finalCompiled .= '
							<span class="discu-minorHeader--last">' . 'Последнее' . '</span>
						</div>
					';
	}
	$__finalCompiled .= '
