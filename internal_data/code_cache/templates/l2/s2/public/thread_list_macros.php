<?php
// FROM HASH: cc6f6d3096c28b36ae0c38fc1ec9ba83
return array(
...
'extensions' => array('icon_cell' => function($__templater, array $__vars, $__extensions = null)
{
	...
	if ($__templater->method($__vars['thread'], 'getUserPostCount', array())) {
		$__finalCompiled .= '
			...
			'data-trigger' => 'auto',
			'title' => 'Вы оставили ' . $__templater->method($__vars['thread'], 'getUserPostCount', array()) . ' сообщений в этой теме',
		))) . '
				';
	}
	$__finalCompiled .= '
...
'thread_type_question_solved' => function($__templater, array $__vars, $__extensions = null)
{
	$__finalCompiled = '';
		$__finalCompiled .= '
							<li>
								<i class="structItem-status structItem-status--solved" aria-hidden="true" title="' . $__templater->filter('Решено', array(array('for_attr', array()),), true) . '"></i>
...
'statuses' => function($__templater, array $__vars, $__extensions = null)
{
	...
	if ($__vars['thread']['discussion_state'] == 'moderated') {
		...
		$__vars['moderatedStatus'] = $__templater->preEscaped('
								<i class="structItem-status structItem-status--moderated" aria-hidden="true" title="' . $__templater->filter('В ожидании одобрения', array(array('for_attr', array()),), true) . '"></i>
								<span class="u-srOnly">' . 'В ожидании одобрения' . '</span>
							');
		$__finalCompiled .= '
							';
	...
	if (!$__vars['thread']['discussion_open']) {
		$__finalCompiled .= '
						<li>
							<i class="structItem-status structItem-status--locked" aria-hidden="true" title="' . $__templater->filter('Закрыта', array(array('for_attr', array()),), true) . '"></i>
	...
	if ($__vars['showWatched'] AND $__vars['xf']['visitor']['user_id']) {
		...
		if ($__vars['thread']['Watch'][$__vars['xf']['visitor']['user_id']]) {
			...
			$__finalCompiled .= '
							<li>
								<i class="structItem-status structItem-status--watched" aria-hidden="true" title="' . $__templater->filter('Отслеживаемый форум', array(array('for_attr', array()),), true) . '"></i>
								<span class="u-srOnly">' . 'Отслеживаемый форум' . '</span>
							</li>
						';
		}
		$__finalCompiled .= '
...
'meta_cell' => function($__templater, array $__vars, $__extensions = null)
{
	...
	if ($__templater->func('property', array('dt_disc_minor_header', ), false)) {
		$__finalCompiled .= '
			<div class="structItem-cell structItem-cell--meta" title="' . $__templater->filter('Количество реакций к первому сообщению' . $__vars['xf']['language']['label_separator'], array(array('for_attr', array()),), true) . ' ' . $__templater->filter($__vars['thread']['first_post_reaction_score'], array(array('number', array()),), true) . '">
				<span class="pairs pairs--justified">
					' . (($__vars['thread']['discussion_type'] == 'redirect') ? '&ndash;' : $__templater->filter($__vars['thread']['reply_count'], array(array('number_short', array()),), true)) . '
				</span>
			</div>
			<div class="structItem-cell structItem-cell--meta" title="' . $__templater->filter('Количество реакций к первому сообщению' . $__vars['xf']['language']['label_separator'], array(array('for_attr', array()),), true) . ' ' . $__templater->filter($__vars['thread']['first_post_reaction_score'], array(array('number', array()),), true) . '">
				<span class="pairs pairs--justified">
					' . (($__vars['thread']['discussion_type'] == 'redirect') ? '&ndash;' : (($__vars['thread']['view_count'] > $__vars['thread']['reply_count']) ? $__templater->filter($__vars['thread']['view_count'], array(array('number_short', array()),), true) : $__templater->func('number_short', array($__vars['thread']['reply_count'] + 1, ), true))) . '
				</span>
			</div>
			';
		...
		if ($__templater->func('property', array('dt_reactions_column', ), false)) {
			$__finalCompiled .= '
					<dl class="pairs pairs--justified structItem-minor">
						<dt>' . 'Реакции' . '</dt>
						<dd>' . $__templater->filter($__vars['thread']['first_post_reaction_score'], array(array('number', array()),), true) . '</dd>
					</dl>
				';
		}
		$__finalCompiled .= '
	...
	$__finalCompiled .= '
			...
			<div class="contentRow-minor contentRow-minor--hideLinks">
				<ul class="listInline listInline--bullet">
					<li>' . 'От: ' . $__templater->escape($__vars['thread']['last_post_cache']['username']) . '' . '</li>
...
'main_cell' => function($__templater, array $__vars, $__extensions = null)
{
	$__finalCompiled = '';
		...
		'title' => 'Создать новую тему в этом форуме',
