...
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Сводка активности');
	$__finalCompiled .= '

';
	if ($__vars['testSuccess'] AND $__vars['xf']['visitor']['email']) {
		$__finalCompiled .= '
	<div class="blockMessage blockMessage--success blockMessage--iconic">' . 'Тестовое сообщение сводки активности успешно отправлено на ' . $__templater->escape($__vars['xf']['visitor']['email']) . '.' . '</div>
';
	}
	$__finalCompiled .= '
...
';
	...
	if (!$__templater->test($__vars['sections'], 'empty', array()) AND $__vars['xf']['visitor']['email']) {
		$__compilerTemp1 .= '
		' . $__templater->button('Отправить тестовое электронное письмо', array(
	...
	<div class="buttonGroup">
		' . $__templater->button('Добавить раздел сводки активности', array(
		...
		if ($__templater->isTraversable($__vars['sections'])) {
			foreach ($__vars['sections'] AS $__vars['section']) {
				$__compilerTemp2 .= '
					...
					'submit' => 'true',
					'tooltip' => 'Включить / отключить \'' . $__vars['section']['title'] . '\'',
					'_type' => 'toggle',
					'html' => '',
				))) . '
					';
