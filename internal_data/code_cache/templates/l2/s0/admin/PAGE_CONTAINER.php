...
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	<div class="menu menu--structural" data-menu="menu" aria-hidden="true">
		<div class="menu-content js-optionGroupsMenuBody">
			<h4 class="menu-header menu-header--small">' . 'Публичный' . '</h4>
	...
	)) . ' ' . 'Добавить модификацию шаблона' . '</a>
	...
	)) . ' ' . 'Добавить элемент навигации' . '</a>
			<hr class="menu-separator menu-separator--hard">

			<h4 class="menu-header menu-header--small">' . 'Панель управления' . '</h4>
			...
			<h4 class="menu-header menu-header--small">' . 'Внешний вид' . '</h4>
			...
			<h4 class="menu-header menu-header--small">' . 'Код' . '</h4>
...
<head>
	...
	<title>
		' . $__templater->func('page_title', array('%s | ' . $__vars['xf']['options']['boardTitle'] . ' - ' . 'Панель управления администратора', $__vars['xf']['options']['boardTitle'] . ' - ' . 'Панель управления администратора', null)) . '
	</title>
...
<div class="p-body-container">
	...
	if ($__vars['listenersDisabled']) {
		$__finalCompiled .= '
					<div class="blockMessage blockMessage--important blockMessage--iconic">
						' . 'Обработчики событий и расширения отключены через config.php. Это разработано как экстренная мера, позволяющая восстановить доступ к панели управления, если плагины блокируют к ней доступ доступ. Это не должно использоваться как инструмент отладки.' . '
					</div>
				';
	}
	$__finalCompiled .= '
	...
	if ($__vars['mailDisabled']) {
		$__finalCompiled .= '
					<div class="blockMessage blockMessage--important blockMessage--iconic">
						' . 'Отправка электронных писем была отключена через файл config.php. Письма не будут отправляться ни одному пользователю до тех пор, пока из config.php не будет удалена строка <code>$config[\'enableMail\'] = false;</code>' . '
					</div>
				';
	}
	$__finalCompiled .= '
...
<footer class="p-footer">
	<div class="p-footer-row">
		<div class="p-footer-row-main">
			<a href="' . $__templater->func('link', array('login/logout', null, array('t' => $__templater->func('csrf_token', array(), false), ), ), true) . '">' . 'Выход' . '</a>
		</div>

		<div class="p-footer-row-opposite">
			<span class="p-footer-copyright">
				' . $__templater->func('copyright') . ' ' . '<div style="text-align: left">Локализация от <a href="https://xenforo.info/" target="_blank">XenForo.Info</a></div>' . '
			</span>
