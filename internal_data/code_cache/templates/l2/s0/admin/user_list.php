...
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Пользователи');
	$__finalCompiled .= '

';
	$__templater->pageParams['pageAction'] = $__templater->preEscaped('
	' . $__templater->button('Экспорт списка пользователей', array(
		...
		'fa' => 'fa-file-export',
		'title' => 'Экспорт CSV-списка всех пользователей, соответствующих текущим параметрам поиска.',
...
';
	...
	';
		...
		if (!$__vars['showingAll']) {
			...
			<div class="block-outer">
				<div class="block-outer-main">
					' . $__templater->button('
						' . 'Сортировка' . $__vars['xf']['language']['label_separator'] . ' ' . ($__templater->escape($__vars['sortOptions'][$__vars['order']]) ?: 'Имя пользователя') . '
					', array(
				...
				'aria-haspopup' => 'true',
					...
					<div class="menu" data-menu="menu" aria-hidden="true">
						<div class="menu-content">
							<h3 class="menu-header">' . 'Сортировка' . $__vars['xf']['language']['ellipsis'] . '</h3>
							';
		...
		if ($__vars['showAll']) {
			$__compilerTemp7 .= '
					<span class="block-footer-controls"><a href="' . $__templater->func('link', array('users/list', null, array('criteria' => $__vars['criteria'], 'all' => true, ), ), true) . '">' . 'Показать все совпадения' . '</a></span>
				';
			...
			$__compilerTemp7 .= '
				...
				'_type' => 'option',
					...
					<span class="block-footer-controls">' . $__templater->button('Массовое обновление', array(
				'type' => 'submit',
			), '', array(
			)) . '</span>
				';
