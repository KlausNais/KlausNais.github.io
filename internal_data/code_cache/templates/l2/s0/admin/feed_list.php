...
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Импорт RSS лент');
	$__finalCompiled .= '

';
	$__templater->pageParams['pageAction'] = $__templater->preEscaped('
	' . $__templater->button('Добавить ленту', array(
		...
		if ($__templater->isTraversable($__vars['feeds'])) {
			foreach ($__vars['feeds'] AS $__vars['feed']) {
				...
				array(
					...
					'submit' => 'true',
					'tooltip' => 'Включить / отключить \'' . $__vars['feed']['title'] . '\'',
					'_type' => 'toggle',
					'html' => '',
				),
				array(
...
';
		...
		$__finalCompiled .= '
	<div class="blockMessage">' . 'Пока не зарегистрировано ни одной ленты.' . '</div>
';
	}
