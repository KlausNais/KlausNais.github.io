	...
	if ($__templater->isTraversable($__compilerTemp1)) {
		...
		';
			...
			', array(
					...
					'hint' => ($__templater->method($__vars['fieldDefinition'], 'isRequired', array($__vars['editMode'], )) ? 'Обязательно' : ''),
...
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	';
		...
		if ($__templater->func('in_array', array($__vars['definition']['match_type'], array('regex', 'alphanumeric', ), ), false)) {
			...
			$__finalCompiled .= '
			';
			$__vars['title'] = $__templater->preEscaped('Пожалуйста, введите значение в соответствии с требуемым форматом.');
			$__finalCompiled .= '
