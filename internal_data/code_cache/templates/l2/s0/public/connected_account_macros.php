...
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	' . $__templater->formRow('
		...
		'text' => 'Связать с ' . $__templater->escape($__vars['provider']['title']) . '',
	), $__vars) . '
	', array(
...
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	$__finalCompiled .= '
	';
	$__vars['explain'] = ((((!$__vars['hasPassword']) AND ($__templater->func('count', array($__vars['xf']['visitor']['Profile']['connected_accounts'], ), false) == 1))) ? 'Разрыв связи со всеми внешними учётными записями приведет к созданию пароля для Вашего аккаунта и его отправке по электронной почте: ' . $__vars['xf']['visitor']['email'] . '.' : '');
	$__finalCompiled .= '
	' . $__templater->formRow('
		...
		'name' => 'disassociate',
		'label' => 'Удалить связь с учётной записью ' . $__templater->escape($__vars['provider']['title']) . '',
		'_dependent' => array('
						' . $__templater->button('Подтвердить разрыв связи', array(
		'type' => 'submit',
	), '', array(
	)) . '
					'),
